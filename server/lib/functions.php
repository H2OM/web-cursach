<?php
    ...
    function HcheckUserInput() {
        ...
        foreach($_POST as $key => $value) {
            ...
            switch($key) {
                case "number":
                    if(!empty($value) && !preg_match("/^\+\d{1}\s\(\d{3}\)\s\d{3}-\d{2}-\d{2}$/", $value)) return false;
                    if(!empty($value)) $dataInputs[$key] = $value;
                    break;
                case "name":
                ...
                case "question":
                    iconv_strlen($value);
                    if(iconv_strlen($value) > 1200 || iconv_strlen($value) < 5) return false;
                    $dataInputs[$key] = $value;
                    break;
                default: break;
            }
