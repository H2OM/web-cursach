    ...
    class _Catalog12 extends Runner {
            ...
            foreach($_GET as $key=>$val) {
                ...
                switch($key) {
                    ...
                    case "favs":
                        ...
                        if(!isset($_COOKIE['favs']) || empty($_COOKIE['favs'])) {
                            break;
                        }
                    ...
                    case "rating":
                        ...
                        if(!empty($rating[0]) && !is_numeric(($rating[0]))) {
                            break;
                        }

                        if(!empty($rating[1]) && !is_numeric(($rating[1]))) {
                            break;
                        }
        ...
        protected function setRating () {
            ...
            try {
                ...
                $params = [["VALUE"=>HgetSafeString($_GET['id']), "PARAMVALUE"=>64]];

                $currentRating = Db::getPreparedQuery("SELECT rating, voices FROM catalog WHERE id = ?", $params);
                ...
                $params = [["VALUE"=>HgetSafeString($newrating), "PARAMVALUE"=>64], ["VALUE"=>HgetSafeString($_GET['id']), "PARAMVALUE"=>64]];

                Db::getPreparedQuery("UPDATE `catalog` SET `rating`= ?, $query WHERE id = ?", $params);
        ...
        protected function getbySearch() {
            ...
            $isContentUperThreeAndLowerFifty = (strlen($_GET['content']) < 3 && strlen($_GET['content']) > 50);

            if(!isset($_GET['content']) || empty($_GET['content']) || $isContentUperThreeAndLowerFifty) {
            ...
            try {
                ...
                $params = [["VALUE"=>$s, "PARAMVALUE"=>64], ["VALUE"=>$s, "PARAMVALUE"=>64]];

                $result = Db::getPreparedQuery("SELECT title, article, image FROM catalog WHERE LOWER(title) LIKE BINARY ? OR  LOWER(city) LIKE BINARY ?", $params);
